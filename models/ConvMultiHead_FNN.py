...
#
#
# 1D Conv Feed Forward Encoder + one fnn head per target
#
#

class CMHFNNCore(nn.Module):
    def __init__(self, hp):
        super().__init__()
        self.hp = hp
        self.encoder = self.build_encoder()
        self.heads = nn.ModuleList([self.build_head() for _ in range(hp.n_targets)])

    def forward(self, x):
        z = self.encoder(x)
        preds = [head(z) for head in self.heads]
        return torch.cat(preds, dim=1)       

    def build_model(self):
        encoder = self.build_encoder()
        heads = [self.build_head() for i in range(self.hp.n_targets)]
...
class ConvMultiHead_FNN(Model_Base):
    def __init__(self, hp):
        super().__init__(hp)

    def _build_model(self):
        return CMHFNNCore(self.hp)
